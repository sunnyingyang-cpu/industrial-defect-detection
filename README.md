...
> 🌐 **[在线展示页](https://sunnyingyang-cpu.github.io/industrial-defect-detection/)**

> 基于 YOLOv5 的工业质检目标检测方案 —— 针对**微小、低对比度、多尺度**表面缺陷优化，  
> 覆盖数据转换 → 训练 → 评测 → **部署 (ONNX / TensorRT / REST API)** 的全流程。
>
> 本项目把一套 YOLOv5 改进方案，落地到**产线表面缺陷检测**这一工业场景，演示从算法到可部署质检服务的完整闭环。
...
> ✅ **本仓库数据集已就绪（本地）**：已拉取 NEU-DET 镜像并跑通转换脚本，当前 `datasets/NEU-DET/` 已含 **1800** 张图（train 1441 / val 179 / test 180），可直接跳到 4.2 训练。原始压缩包与解包目录保留在 `datasets/NEU-DET-raw/`。
